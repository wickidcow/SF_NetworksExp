...
public class AutoCrafter extends NetworkObject implements SoftCellBannable, CraftTyped {
    ...
    protected boolean craftPreFlight(@NotNull BlockMenu blockMenu) {
        ...
        final int requestedCrafts = canBlueprintStack() ? Math.max(1, blueprint.getAmount()) : 1;
        ...
        /*
         * Advanced Auto Crafters use the encoded-blueprint stack size as their requested batch size.
         * The original addon documented this as "stackable blueprints". Clamp that request to the
         * remaining room in one legal output stack instead of rejecting recipes whose per-craft output
