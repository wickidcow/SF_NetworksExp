...
ORIGINAL_ITEMS: dict[str, tuple[str, list[str]]] = {
    ...
    "NTW_MONITOR": ("Network Monitor", ["Allows the network to interact with", "an adjacent supported inventory.", "", "Shows every node the Network Controller sees.", "Hover machine types for Total / Active / Inactive.", "Click a type for individual nodes and status.", "Click a node to highlight it in-world.", "Use Refresh to force topology rediscovery.", "Storage direction is set from the bottom toolbar."]),
